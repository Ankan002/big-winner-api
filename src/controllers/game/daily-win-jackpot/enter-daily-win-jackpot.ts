...
export const enterDailyWinJackpot = async (req: Request, res: Response) => {
	const user = req.user;
	...
	try {
		const lotteryContest = await prismaInstance.dailyWinJackpot.findFirst({
			where: {
				status: "open",
			},
			select: {
		...
		const earlierBet = await prismaInstance.dailyWinJackpotEntry.findFirst({
			where: {
		...
		const createBetPromise = prismaInstance.dailyWinJackpotEntry.create({
			data: {
